...
export function AppShell({ children, userName, userRole }: { children: ReactNode; userName?: string; userRole?: string }) {
  return (
      ...
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-76 border-r border-white/10 bg-navy-900 text-white shadow-2xl lg:block">
        <div className="border-b border-white/10 px-6 py-6">
          ...
          <div className="mt-5 rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
            <p className="text-xs text-navy-100">Signed in operations user</p>
            <div className="mt-2 flex items-center justify-between gap-3">
              <div>
                <p className="truncate text-sm font-semibold text-white">{userName ?? "Portal user"}</p>
                <p className="mt-1 text-xs text-green-200">{userRole ?? "verified role"}</p>
              </div>
        ...
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 px-4 py-4 shadow-sm backdrop-blur lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="hidden rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600 sm:inline-flex">Supabase Auth</span>
              <LogoutButton />
