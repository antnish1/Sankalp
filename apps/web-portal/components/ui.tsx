...
export function SearchFilterBar({
  ...
  query = "",
  filter = "all",
  filterOptions = ["active", "attention", "closed"]
}: {
  ...
  return (
    <form className="mb-4 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm" action="" method="get">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="relative flex-1">
          <span className="pointer-events-none absolute left-3 top-2.5 text-slate-400">⌕</span>
          <input className="w-full pl-9" name="q" placeholder={searchPlaceholder} aria-label={searchPlaceholder} defaultValue={query} />
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <select className="min-w-40" name="status" aria-label={filterLabel} defaultValue={filter}>
            <option value="all">All {filterLabel.toLowerCase()}</option>
            {filterOptions.map((option) => <option value={option} key={option}>{option}</option>)}
          </select>
          <button className="rounded-xl bg-navy-700 px-4 py-2 text-sm font-semibold text-white" type="submit">Apply</button>
          <a className="rounded-xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-700" href="?">Clear</a>
          {action}
        </div>
      </div>
    </form>
